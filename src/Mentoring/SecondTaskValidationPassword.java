...
public class SecondTaskValidationPassword {
	...
	public static boolean Validation(String password) {
		...
		if (password.contains(" ")) {
			...
			//and our method will not continue running the code after that. No sense? that its already false.
			return result;
			
			//if first statement was false (that means that password doesnt have spaces)
			//check the length of password? if its less  the 6, same return result (false by default)
		...
		count+=2;
		 ...
		 // Creating the array and use the method toCharArraym i converting the main string to char array. Why?
		char[] passWord = password.toCharArray();
		
		//Because i will compare digits from string with digits from int,and i can use parseInt method,
		//but i decided that way. Here as example ? what should i type if i want to check text(string) and digit(int)
...
//    	System.out.println(a.equals(b));// false, should be true
		...
		//Why i using char as initializer , because our string was converted char array.
		for (char i = '0'; i <= '9'; i++) {
			...
			for (int j = 0; j < passWord.length; j++) {
				...
				//mine char array has an at least one value from char i variable. We are looking for digit
				if (passWord[j] == i) {
				...
				//then check the value of count, it must be equals to count of passes paragraphs.
		...
		if (count == 3) {
			...
			//create the for loop, and check, does have a at least one lower case letter our password.
			...
			for (char l = 'a'; l < 'z'; l++) {
				...
				for (int i = 0; i < passWord.length; i++) {
					...
					//Creating the if statement inside that loop, and check that
					//mine char array has an at least one value from char i variable. We are looking for lower case letter
					if (passWord[i] == l) {
				...
				//then check the value of count, it must be equals to count of passes requirements.
			...
			//here , if our first if statement, before the for loop, which check the count of passed requirements will be false
			//we go under else statement and exit from the method, Java will stop runs the code
		} else {
			return result = false;
		}
		...
		if (count == 4) {
			...
			//create the for loop, and check, does have a at least one upper case letter our password.
			//the condition we can use a last letter of the alphabet
			for (char U = 'A'; U <= 'Z'; U++) {
				...
				for (int i = 0; i < passWord.length; i++) {
					...
					//Creating the if statement inside that loop, and check that
					//mine char array has an at least one value from char i variable. We are looking for upper case letter
					if (passWord[i] == U) {
						//if its pass , increase value of count variable on 1;
						count++;
		...
		if (count == 5) {
			...
			//create the for loop, and check, does have a at least one special character in our password.
			...
			for (int s = 0; s < SC.length; s++) {
				...
				for (int i = 0; i < passWord.length; i++) {
					...
					//Creating the if statement inside that loop, and check that
					// char array where we saved the password has an at least one value from char array where we saved the special characters.
		...
		//if we are came to that line, that means that our requirements were passed and last if statement 
		//changed the value of our boolean variable from false to true. Returning boolean value we are returning true
		return result;
	}
	...
	//main method where we will call the method which will check the requirements for password
